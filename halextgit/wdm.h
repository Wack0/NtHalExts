...
_interlockedbittestandset (
    _Inout_updates_bytes_((Offset+7)/8) __drv_interlocked LONG volatile *Base,
    _In_ LONG Offset
    );
...
InterlockedBitTestAndSet (
    _Inout_updates_bytes_((Bit+7)/8) __drv_interlocked LONG volatile *Base,
    _In_ LONG Bit
    )
{
    __asm {
...
SHORT
_InterlockedAnd16 (
    _Inout_ __drv_interlocked SHORT volatile *Destination,
    _In_ SHORT Value
    );
...
_interlockedbittestandset64 (
    _Inout_updates_bytes_((Offset+7)/8) __drv_interlocked LONG64 volatile *Base,
    _In_ LONG64 Offset
    );
...
InterlockedIncrement16 (
    _Inout_ __drv_interlocked SHORT volatile *Addend
    );
...
LONG
InterlockedAnd (
    _Inout_ __drv_interlocked LONG volatile *Destination,
    _In_ LONG Value
    );
...
LONG64
InterlockedAnd64 (
    _Inout_ __drv_interlocked LONG64 volatile *Destination,
    _In_ LONG64 Value
    );
...
InterlockedIncrement (
    _Inout_ __drv_interlocked LONG volatile *Addend
    );
...
LONG
InterlockedExchange (
    _Inout_ __drv_interlocked LONG volatile *Target,
    _In_ LONG Value
    );
...
InterlockedIncrement64 (
    _Inout_ __drv_interlocked LONG64 volatile *Addend
    );
...
InterlockedExchange64 (
    _Inout_ __drv_interlocked LONG64 volatile *Target,
    _In_ LONG64 Value
    );
...
__out_xcount(Unknown) PVOID
InterlockedCompareExchangePointer (
    __deref_inout_xcount(Unknown) __drv_interlocked PVOID volatile *Destination,
...
InterlockedExchangePointer(
    __deref_inout_xcount(Unknown) __drv_interlocked PVOID volatile *Target,
...
InterlockedExchange8 (
    _Inout_ __drv_interlocked CHAR volatile *Target,
    _In_ CHAR Value
    );
...
char
InterlockedAnd8 (
    _Inout_ __drv_interlocked char volatile *Destination,
    _In_ char Value
    );
...
//
// Define legacy floating point save/restore intrinsics.
//

#if _MSC_FULL_VER >= 161030716

//
// ISSUE: 2010-Oct-31-neerajsi- Workaround for VS RAID#76134 -
//        "Incorrect code generation for _fxsave intrinsics."
//        Remove memory barriers when the bug is fixed.
//

#define FxstateRestoreIntrinsic _fxrstor
#define FxstateRestore64Intrinsic _fxrstor64
#define FxstateSaveIntrinsic _fxsave
#define FxstateSave64Intrinsic _fxsave64
...

{
    ...
    _ReadWriteBarrier();
    FxstateRestoreIntrinsic(SaveArea);
    _ReadWriteBarrier();
}
...

{
    ...
    _ReadWriteBarrier();
    FxstateRestore64Intrinsic(SaveArea);
    _ReadWriteBarrier();
}
...

{
    ...
    _ReadWriteBarrier();
    FxstateSave64Intrinsic(SaveArea);
    _ReadWriteBarrier();
}
...
//
// _ARM_WORKITEM_ update for v6/v7 ops
//
...
PVOID
InterlockedCompareExchangePointer (
    _Inout_ __drv_interlocked PVOID volatile *Destination,
    _In_opt_ PVOID Exchange,
    _In_opt_ PVOID Comperand
    );
...
PVOID
InterlockedExchangePointer(
    _Inout_ __drv_interlocked PVOID volatile *Target,
    _In_opt_ PVOID Value
    );
...

{
    ...
    PreviousFlags = ReadForWriteAccess((volatile SHORT *)Destination);
    do {
...
#define CP15_CR9_PMSELR         15, 0, 9, 12, 5
#define CP15_CR9_PMXEVCNTR      15, 0, 9, 13, 2
#define CP15_CR13_THPR_USRRW    15, 0, 13, 0, 2
#define CP15_CR13_THPR_USRRO    15, 0, 13, 0, 3
#define CP15_CR13_THPR_SVCRW    15, 0, 13, 0, 4
...
#else
#define MemoryBarrier()                      __emit(0xF57FF05F)
#define _DataSynchronizationBarrier()        __emit(0xF57FF04F)
#define _InstructionSynchronizationBarrier() __emit(0xF57FF06F)
#endif

#define _InvalidateBTAC() _MoveToCoprocessor(0, CP15_CR7_INVAL_BTAC)
...
#define PF_NON_TEMPORAL_LEVEL_ALL 1
...

{
    ...
    _MoveToCoprocessor(Counter, CP15_CR9_PMSELR);
    return (ULONG64)_MoveFromCoprocessor(CP15_CR9_PMXEVCNTR);
}
...
__cdecl
InterlockedAdd64 (
    _Inout_ __drv_interlocked LONGLONG volatile *Addend,
    _In_ LONGLONG Value
    );
...
__cdecl
InterlockedIncrement16 (
    _Inout_ __drv_interlocked volatile SHORT *Addend
    );
...
__cdecl
InterlockedExchange64(
    _Inout_ __drv_interlocked LONGLONG volatile *Target,
    _In_ LONGLONG Value
    );
...
__cdecl
InterlockedCompareExchange64 (
    _Inout_ __drv_interlocked LONGLONG volatile *Destination,
...
InterlockedCompare64Exchange128(
    _Inout_updates_bytes_(16) __drv_interlocked LONG64 volatile *Destination,
    _In_ LONG64 ExchangeHigh,
    _In_ LONG64 ExchangeLow,
    _In_ LONG64 Comperand
    );
...
InterlockedBitTestAndSet_Inline (
    _Inout_ __drv_interlocked LONG volatile *Base,
    _In_ LONG Bit
    )
{
    LONG tBit;
...
InterlockedBitTestAndSet64_Inline (
    _Inout_ __drv_interlocked LONG64 volatile *Base,
    _In_ LONG64 Bit
    )
{
    LONG64 tBit;
...
RtlQueryRegistryValues(
    ...
    _In_     PCWSTR Path,
    _Inout_ _At_(*(*QueryTable).EntryContext, __post __valid)
...
__drv_maxIRQL(PASSIVE_LEVEL)
_When_(AllocateDestinationString, _Check_return_)
NTSYSAPI
NTSTATUS
NTAPI
...
__cdecl
DbgPrint (
    _In_z_ __drv_formatString(printf) PCSTR Format,
    ...
    );
...
DbgPrintReturnControlC (
    _In_z_ __drv_formatString(printf) PCCH Format,
    ...
    );
#endif
...
NTAPI
RtlGetVersion(
    _Out_ _At_(lpVersionInformation->dwOSVersionInfoSize,  _Inout_)
...
//
// Use __struct_xcount until the definition can be fixed (PAGE_SIZE may not be
// defined at this point).
//

typedef
    __struct_xcount(sizeof(struct _MDL) +    // 747934
...
#define TIMER_TOLERABLE_DELAY_BITS      4
...
#define ASSERT_EVENT(E)                                                      \
    NT_ASSERT((KOBJECT_TYPE(E) == EventNotificationObject) ||                \
              (KOBJECT_TYPE(E) == EventSynchronizationObject))
...
FASTCALL
KfLowerIrql (
    _In_ __drv_restoresIRQL __drv_nonConstant KIRQL NewIrql
    );
#endif
...
NTAPI
READ_REGISTER_UCHAR(
    _In_ __drv_nonConstant PUCHAR  Register
    );
...
READ_REGISTER_USHORT(
    _In_ __drv_nonConstant PUSHORT Register
    );
...
NTAPI
READ_REGISTER_ULONG(
    _In_ __drv_nonConstant PULONG  Register
    );
...
NTAPI
READ_PORT_UCHAR(
    _In_ __drv_nonConstant PUCHAR  Port
    );
...
NTAPI
READ_PORT_USHORT(
    _In_ __drv_nonConstant PUSHORT Port
    );
...
NTAPI
READ_PORT_ULONG(
    _In_ __drv_nonConstant PULONG  Port
    );
...
__drv_maxIRQL(DISPATCH_LEVEL)
__drv_valueIs(<0;==0)
_When_(return==0, __drv_floatSaved)
NTKERNELAPI
NTSTATUS
NTAPI
KeSaveFloatingPointState (
    _Out_ __deref __drv_neverHold(FloatState)
    _When_(return==0, __deref __drv_acquiresResource(FloatState))
...
KeRestoreFloatingPointState (
    _In_ __deref __drv_releasesExclusiveResource(FloatState)
    PKFLOATING_SAVE FloatSave
    );
...
UCHAR
READ_REGISTER_UCHAR (
    _In_ __drv_nonConstant volatile UCHAR *Register
    )
{
    _ReadWriteBarrier();
    return *Register;
...
USHORT
READ_REGISTER_USHORT (
    _In_ __drv_nonConstant volatile USHORT *Register
    )
{
    _ReadWriteBarrier();
    return *Register;
...
ULONG
READ_REGISTER_ULONG (
    _In_ __drv_nonConstant volatile ULONG *Register
    )
{
    _ReadWriteBarrier();
    return *Register;
...
READ_REGISTER_ULONG64 (
    _In_ __drv_nonConstant volatile ULONG64 *Register
    )
{
    _ReadWriteBarrier();
    return *Register;
...
VOID
READ_REGISTER_BUFFER_UCHAR (
    _In_reads_(Count) __drv_nonConstant PUCHAR Register,
    __out_ecount_full(Count) PUCHAR Buffer,
    _In_ ULONG Count
    )
{
...
VOID
READ_REGISTER_BUFFER_USHORT (
    _In_reads_(Count) __drv_nonConstant PUSHORT Register,
    __out_ecount_full(Count) PUSHORT Buffer,
    _In_ ULONG Count
    )
{
...
VOID
READ_REGISTER_BUFFER_ULONG (
    _In_reads_(Count) __drv_nonConstant PULONG Register,
    __out_ecount_full(Count) PULONG Buffer,
    _In_ ULONG Count
    )
{
...
READ_REGISTER_BUFFER_ULONG64 (
    _In_reads_(Count) __drv_nonConstant PULONG64 Register,
...
VOID
WRITE_REGISTER_BUFFER_UCHAR (
    _Out_writes_(Count) __drv_nonConstant PUCHAR Register,
    _In_reads_(Count) PUCHAR Buffer,
    _In_ ULONG Count
    )
{
...
VOID
WRITE_REGISTER_BUFFER_USHORT (
    _Out_writes_(Count) __drv_nonConstant PUSHORT Register,
    _In_reads_(Count) PUSHORT Buffer,
    _In_ ULONG Count
    )
{
...
VOID
WRITE_REGISTER_BUFFER_ULONG (
    _Out_writes_(Count) __drv_nonConstant PULONG Register,
    _In_reads_(Count) PULONG Buffer,
    _In_ ULONG Count
    )
{
...
WRITE_REGISTER_BUFFER_ULONG64 (
    _Out_writes_(Count) __drv_nonConstant PULONG64 Register,
...
NTSTATUS
KeRestoreFloatingPointState (
    _In_ __deref __drv_releasesExclusiveResource(FloatState) PVOID FloatingState
    )
...
VOID
KeLowerIrql (
    _In_ __drv_nonConstant __drv_restoresIRQL KIRQL NewIrql
   );
...
#define CP15_CR7_PREFETCHFLUSH 15, 0,  7,  5, 4
#define CP15_CR7_INVAL_BTAC    15, 0,  7,  5, 6
#define CP15_CR7_INVAL_BTAC_LINE 15,0, 7,  5, 7

//
// Memory Barriers
//

#define CP15_CR7_DSB           15, 0,  7, 10, 4
#define CP15_CR7_DMB           15, 0,  7, 10, 5
...
KeRaiseIrql (
    _In_ KIRQL NewIrql,
    _Out_ __drv_savesIRQL PKIRQL OldIrql
    );
...
KeAcquireInterruptSpinLock (
    _Inout_ __deref __drv_acquiresExclusiveResource(InterruptSpinLock)
...
KeReleaseInterruptSpinLock (
    _Inout_ __deref __drv_releasesExclusiveResource(InterruptSpinLock)
    PKINTERRUPT Interrupt,
    _In_ __drv_restoresIRQL KIRQL OldIrql
    );
#endif
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
_When_(Wait==0, __drv_maxIRQL(DISPATCH_LEVEL))
_When_(Wait==1, __drv_maxIRQL(APC_LEVEL))
...
KeSetEvent (
    ...
    _In_ __drv_constant BOOLEAN Wait
    );
#endif
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
__drv_minIRQL(PASSIVE_LEVEL)
_When_((Timeout==NULL || *Timeout!=0), __drv_maxIRQL(APC_LEVEL))
_When_((Timeout!=NULL && *Timeout==0), __drv_maxIRQL(DISPATCH_LEVEL))
NTKERNELAPI
NTSTATUS
...
KeWaitForSingleObject (
    _In_ __deref __drv_notPointer PVOID Object,
    _In_ __drv_strictTypeMatch(__drv_typeCond) KWAIT_REASON WaitReason,
    _In_ __drv_strictType(KPROCESSOR_MODE/enum _MODE,__drv_typeConst)
    KPROCESSOR_MODE WaitMode,
    _In_ BOOLEAN Alertable,
...
KeTryToAcquireSpinLockAtDpcLevel (
    _Inout_ __deref __drv_neverHold(KeSpinLockType)
    _When_(return!=0, __deref __drv_acquiresResource(KeSpinLockType))
...
KefAcquireSpinLockAtDpcLevel (
    _Inout_ __deref __drv_acquiresExclusiveResource(KeSpinLockType)
    PKSPIN_LOCK SpinLock
    );
#endif
...
KefReleaseSpinLockFromDpcLevel (
    _Inout_ __deref __drv_releasesExclusiveResource(KeSpinLockType)
    PKSPIN_LOCK SpinLock
    );
#endif
...
KfReleaseSpinLock (
    _Inout_ __deref __drv_releasesExclusiveResource(KeSpinLockType)
    PKSPIN_LOCK SpinLock,
    _In_ __drv_restoresIRQL KIRQL NewIrql
    );
#endif
...
KeAcquireInStackQueuedSpinLock (
    ...
    _Out_ __deref __drv_acquiresExclusiveResource(KeQueuedSpinLockType)
...
KeReleaseInStackQueuedSpinLock (
    _In_ __deref __drv_releasesExclusiveResource(KeQueuedSpinLockType)
    PKLOCK_QUEUE_HANDLE LockHandle
    );
#endif
...
KeAcquireGuardedMutex (
    _Inout_ __deref __drv_neverHold(GuardedMutex) __deref __drv_acquiresResource(GuardedMutex)
...
KeReleaseGuardedMutex (
    _Inout_ __deref __drv_releasesExclusiveResource(GuardedMutex)
    PKGUARDED_MUTEX Mutex
    );
#endif
...
KeTryToAcquireGuardedMutex (
    _Inout_ __deref __drv_neverHold(GuardedMutex)
    __deref _When_(return==1, __drv_acquiresResource(GuardedMutex))
    PKGUARDED_MUTEX Mutex
    );
#endif
...
KeAcquireGuardedMutexUnsafe (
    _Inout_ __deref __drv_neverHold(KeFastMutex) __deref __drv_acquiresResource(KeFastMutex)
...
KeReleaseGuardedMutexUnsafe (
    _Inout_ __deref __drv_releasesExclusiveResource(KeFastMutex)
...
KeSaveExtendedProcessorState (
    _In_ ULONG64 Mask,
    _Out_ __deref __drv_neverHold(XStateSave)
    _When_(return==0, __deref __drv_acquiresResource(XStateSave))
...
KeRestoreExtendedProcessorState (
    _In_ __deref __drv_releasesExclusiveResource(XStateSave)
    PXSTATE_SAVE XStateSave
    );
#endif
...
// FUTURE: _ARM_ to be removed from exclusion list pending additional work

#if !defined(_IA64_) && !defined(_X86_) && !defined(_AMD64_) && !defined(_NTOS_)
...
_When_((PoolType & PagedPool) != 0, __drv_maxIRQL(APC_LEVEL))
_When_((PoolType & PagedPool) == 0, __drv_maxIRQL(DISPATCH_LEVEL))
_When_((PoolType & NonPagedPoolMustSucceed) != 0,
           __drv_reportError("Must succeed pool allocations are forbidden. "
                             "Allocation failures cause a system crash"))
_When_((PoolType & (NonPagedPoolMustSucceed |
                        POOL_RAISE_IF_ALLOCATION_FAILURE)) == 0,
           __post __maybenull _Check_return_)
_When_((PoolType & (NonPagedPoolMustSucceed |
                        POOL_RAISE_IF_ALLOCATION_FAILURE)) != 0,
           __post __notnull)
...
ExAcquireFastMutexUnsafe(
    _Inout_ __deref __drv_acquiresExclusiveResource(FastMutexType)
...
ExReleaseFastMutexUnsafe(
    _Inout_ __deref __drv_releasesExclusiveResource(FastMutexType)
    PFAST_MUTEX FastMutex
    );
...
ExInterlockedAddLargeInteger (
    ...
    _Inout_ __deref __drv_neverHold(KeSpinLockType) PKSPIN_LOCK Lock
    );
...
NTAPI
ProbeForRead (
    __in_data_source(USER_MODE) __out_validated(MEMORY) _In_reads_bytes_(Length)
    PVOID Address,
    _In_ SIZE_T Length,
...
NTAPI
ProbeForWrite (
    __in_data_source(USER_MODE) __out_validated(MEMORY) _Inout_updates_bytes_(Length)
    PVOID Address,
    _In_ SIZE_T Length,
    _In_ ULONG Alignment
    );
...
__drv_maxIRQL(APC_LEVEL)
__drv_mustHoldCriticalRegion
_When_(Wait!=0, __drv_valueIs(==1))
_When_(Wait==0, __drv_valueIs(==0;==1) _Check_return_)
NTKERNELAPI
BOOLEAN
ExAcquireResourceSharedLite (
    _Inout_ __deref __drv_neverHold(ExResourceType)
    __deref _When_(return==1, __drv_acquiresResource(ExResourceType))
    PERESOURCE Resource,
    _In_ BOOLEAN Wait
    );
...
ExEnterCriticalRegionAndAcquireResourceShared (
    _Inout_ __deref __drv_acquiresExclusiveResource(ExResourceType)
    PERESOURCE Resource
    );
...
ExAcquireSharedStarveExclusive(
    _Inout_ __deref __drv_neverHold(ExResourceType)
    __deref _When_(return!=0, __drv_acquiresResource(ExResourceType))
    PERESOURCE Resource,
    _In_ BOOLEAN Wait
    );
...
ExReleaseResourceLite(
    _Inout_ __deref __drv_releasesExclusiveResource(ExResourceType)
    PERESOURCE Resource
    );
...
ExConvertExclusiveToSharedLite(
    _Inout_ __deref __drv_mustHold(ExResourceType) PERESOURCE Resource
    );
...
_When_(AccessMode==0, __drv_maxIRQL(DISPATCH_LEVEL))
_When_(AccessMode==1, __drv_inTry __drv_maxIRQL(APC_LEVEL)) 
...
_When_(AccessMode==KernelMode, __drv_maxIRQL(DISPATCH_LEVEL))
_When_(AccessMode==UserMode, __drv_inTry __drv_maxIRQL(APC_LEVEL) __notnull)
_At_(MemoryDescriptorList->MappedSystemVa, 
    __post __bcount(MemoryDescriptorList->ByteCount)) // Esp:829
_Check_return_ 
...
PMDL
MmCreateMdl (
    _In_opt_ PMDL MemoryDescriptorList,
    _In_reads_bytes_opt_ (Length) PVOID Base,
    _In_ SIZE_T Length
    );
#endif
...
SeSetAuditParameter(
    ...
    __in_xcount("depends on SE_ADT_PARAMETER_TYPE") PVOID Data
    );
#endif
...
DRIVER_CANCEL (
    ...
    _Inout_ __drv_useCancelIRQL struct _IRP *Irp
    );
...
__drv_requiresIRQL(PASSIVE_LEVEL)
__drv_sameIRQL
_When_(return>=0, __drv_clearDoInit(__yes))
typedef
NTSTATUS
DRIVER_ADD_DEVICE (
    _In_ struct _DRIVER_OBJECT *DriverObject,
...
IoAcquireCancelSpinLock(
    _Out_ __deref __drv_savesIRQL PKIRQL Irql
    );
#endif
...
IoAllocateDriverObjectExtension(
    ...
    __post __deref _When_(return==0,
    __drv_aliasesMem __drv_allocatesMem(Mem) __drv_valueIs(!=0))
...
NTSTATUS
IoAttachDevice(
    _In_ __drv_mustHold(Memory) _When_(return==0, __drv_aliasesMem)
...
IoCreateDevice(
    ...
    __drv_out_deref(
        __drv_allocatesMem(Mem)
        _When_((((inFunctionClass$("DRIVER_INITIALIZE"))
...
VOID
IoDeleteDevice(
    _In_ __drv_mustHold(Memory) __drv_freesMem(Mem) PDEVICE_OBJECT DeviceObject
    );
#endif
...
IoGetDmaAdapter(
    ...
    _Out_ _When_(return!=0, __drv_IoGetDmaAdapter __deref _Check_return_)
...
IoReleaseCancelSpinLock(
    _In_ __drv_restoresIRQL __drv_useCancelIRQL KIRQL Irql
    );
#endif
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
NTKERNELAPI
_When_(ControlCode==EVENT_ACTIVITY_CTRL_CREATE_ID, __drv_maxIRQL(HIGH_LEVEL))
_When_(ControlCode!=EVENT_ACTIVITY_CTRL_CREATE_ID, __drv_maxIRQL(APC_LEVEL))
NTSTATUS
...
IoRegisterPlugPlayNotification(
    ...
    _Outptr_ 
    __drv_deref(
        _When_(return==0, __drv_allocatesMem(Mem) __drv_valueIs(!=0)) 
        _When_(return<0, __drv_valueIs(==0)))
...
typedef VOID
__drv_functionClass(DMA_COMPLETION_ROUTINE)
__drv_maxIRQL(DISPATCH_LEVEL)
__drv_minIRQL(DISPATCH_LEVEL)
...
typedef NTSTATUS
(*PTRANSFER_ADAPTER_CHANNEL)(
    _In_ PDMA_ADAPTER CurrentOwner,
    _In_ PDMA_ADAPTER NextOwner
    );
...
typedef struct _DMA_OPERATIONS {
    ...
    PTRANSFER_ADAPTER_CHANNEL TransferAdapterChannel;
...
#define POWER_RUNTIME_MANAGEMENT_VERSION_V1 0x00000001

DECLARE_HANDLE(POHANDLE);

typedef
__drv_functionClass(POWER_RUNTIME_COMPONENT_IDLE_STATE_CALLBACK)
__drv_maxIRQL(DISPATCH_LEVEL)
VOID
POWER_RUNTIME_COMPONENT_IDLE_STATE_CALLBACK (
    _In_ PVOID Context,
    _In_ ULONG Component,
    _In_ ULONG State
    );

typedef POWER_RUNTIME_COMPONENT_IDLE_STATE_CALLBACK
    *PPOWER_RUNTIME_COMPONENT_IDLE_STATE_CALLBACK;

typedef
__drv_functionClass(POWER_RUNTIME_COMPONENT_ACTIVE_CONDITION_CALLBACK)
__drv_maxIRQL(DISPATCH_LEVEL)
VOID
POWER_RUNTIME_COMPONENT_ACTIVE_CONDITION_CALLBACK (
    _In_ PVOID Context,
    _In_ ULONG Component,
    _In_ BOOLEAN Active
    );

typedef POWER_RUNTIME_COMPONENT_ACTIVE_CONDITION_CALLBACK
    *PPOWER_RUNTIME_COMPONENT_ACTIVE_CONDITION_CALLBACK;

typedef
__drv_functionClass(POWER_RUNTIME_COMPONENT_PERF_STATE_CALLBACK)
__drv_maxIRQL(DISPATCH_LEVEL)
VOID
POWER_RUNTIME_COMPONENT_PERF_STATE_CALLBACK (
    _In_ PVOID Context,
    _In_ ULONG Component,
    _In_ ULONG Performance,
    _In_ BOOLEAN Pause
    );

typedef POWER_RUNTIME_COMPONENT_PERF_STATE_CALLBACK
    *PPOWER_RUNTIME_COMPONENT_PERF_STATE_CALLBACK;

typedef
__drv_functionClass(POWER_RUNTIME_DEVICE_SUSPENDED_CONDITION_CALLBACK)
__drv_maxIRQL(DISPATCH_LEVEL)
VOID
POWER_RUNTIME_DEVICE_SUSPENDED_CONDITION_CALLBACK (
    _In_ PVOID Context,
    _In_ BOOLEAN Suspended
    );

typedef POWER_RUNTIME_DEVICE_SUSPENDED_CONDITION_CALLBACK
    *PPOWER_RUNTIME_DEVICE_SUSPENDED_CONDITION_CALLBACK;

typedef
__drv_functionClass(POWER_RUNTIME_DEVICE_POWER_REQUIRED_CALLBACK)
__drv_maxIRQL(DISPATCH_LEVEL)
VOID
POWER_RUNTIME_DEVICE_POWER_REQUIRED_CALLBACK (
    _In_ PVOID Context,
    _In_ BOOLEAN PowerRequired
    );

typedef POWER_RUNTIME_DEVICE_POWER_REQUIRED_CALLBACK
    *PPOWER_RUNTIME_DEVICE_POWER_REQUIRED_CALLBACK;

struct _POWER_RUNTIME_COMPONENT_IDLE_STATE {
    ULONG TransitionLatency;
    ULONG ResidencyRequirement;
    ULONG NominalPower;
};

typedef struct _POWER_RUNTIME_COMPONENT_IDLE_STATE
    POWER_RUNTIME_COMPONENT_IDLE_STATE,
    *PPOWER_RUNTIME_COMPONENT_IDLE_STATE;

enum _POWER_RUNTIME_PERF_STATE_TYPE {
    PoPerfTypeNone = 0,
    PoPerfTypeRange = 1,
    PoPerfTypeDiscrete = 2,
    PoPerfTypeMax
};

typedef enum _POWER_RUNTIME_PERF_STATE_TYPE
    POWER_RUNTIME_PERF_STATE_TYPE, 
    *PPOWER_RUNTIME_PERF_STATE_TYPE;

struct _POWER_RUNTIME_COMPONENT_PERF_STATE {
    ULONG Performance;
    ULONG NominalPower;
    ULONG RequiredFrequency;
};

typedef struct _POWER_RUNTIME_COMPONENT_PERF_STATE
    POWER_RUNTIME_COMPONENT_PERF_STATE,
    *PPOWER_RUNTIME_COMPONENT_PERF_STATE;

struct _POWER_RUNTIME_COMPONENT {
    GUID Id;
    ULONG IdleStateCount;
    _Field_size_full_(IdleStateCount)
        PPOWER_RUNTIME_COMPONENT_IDLE_STATE IdleStates;
    ULONG LastWakeableIdleState;
    POWER_RUNTIME_PERF_STATE_TYPE PerfStateType;
    BOOLEAN PerfHWInternal;
    union {
        struct {
            ULONG MinimumPerformance;
            ULONG MaximumPerformance;
            ULONG PerformanceStep;
        };
        struct {
            ULONG PerfStateCount;
            PPOWER_RUNTIME_COMPONENT_PERF_STATE PerfStates;
        };
    };
};

typedef struct _POWER_RUNTIME_COMPONENT POWER_RUNTIME_COMPONENT, *PPOWER_RUNTIME_COMPONENT;

struct _POWER_RUNTIME_MANAGEMENT {
    ULONG Version;
    ULONG ComponentCount;
    PPOWER_RUNTIME_COMPONENT_IDLE_STATE_CALLBACK ComponentIdleStateCallback;
    PPOWER_RUNTIME_COMPONENT_PERF_STATE_CALLBACK ComponentPerfStateCallback;
    PPOWER_RUNTIME_COMPONENT_ACTIVE_CONDITION_CALLBACK ComponentActiveConditionCallback;
    PPOWER_RUNTIME_DEVICE_SUSPENDED_CONDITION_CALLBACK DeviceSuspendedConditionCallback;
    PPOWER_RUNTIME_DEVICE_POWER_REQUIRED_CALLBACK DevicePowerRequiredCallback;
    PVOID RuntimeContext;
    _Field_size_full_(ComponentCount) POWER_RUNTIME_COMPONENT Components[ANYSIZE_ARRAY];
};

typedef struct _POWER_RUNTIME_MANAGEMENT POWER_RUNTIME_MANAGEMENT, *PPOWER_RUNTIME_MANAGEMENT;

//
// Driver interfaces for runtime framework.
//

#define PO_RUNTIME_FLAG_BLOCKING   0x1
#define PO_RUNTIME_FLAG_ASYNC_ONLY 0x2

#if (NTDDI_VERSION >= NTDDI_WIN8)
__drv_maxIRQL(PASSIVE_LEVEL)
NTKERNELAPI
NTSTATUS
PoRegisterCoreRuntimeManagement (
    __in PCUNICODE_STRING Id,
    __in PPOWER_RUNTIME_MANAGEMENT RuntimeManagement,
...
NTKERNELAPI
NTSTATUS
PoRegisterRuntimeManagement (
    _In_ PDEVICE_OBJECT Pdo,
    _In_ PPOWER_RUNTIME_MANAGEMENT RuntimeManagement,
...
NTKERNELAPI
VOID
PoRuntimeSetResidency (
    _In_ POHANDLE Handle,
    _In_ ULONG Component,
    _In_ ULONG Residency
    );
#endif
...
typedef
VOID
POCALLBACKREQUESTWORKER (
    _In_ POHANDLE PluginHandle
    );

typedef POCALLBACKREQUESTWORKER *PPOCALLBACKREQUESTWORKER;

#define PEP_KERNEL_INFORMATION_V1 0x00000001

struct _PEP_KERNEL_INFORMATION {
    USHORT Version;
    USHORT Size;
    POHANDLE Plugin;
    PPOCALLBACKREQUESTWORKER RequestWorker;
};

typedef struct _PEP_KERNEL_INFORMATION PEP_KERNEL_INFORMATION, *PPEP_KERNEL_INFORMATION;

#define PEP_CALLBACKS_V1 0x00000001

typedef
BOOLEAN
PEPCALLBACKPREPAREDEVICE (
    _In_ PCUNICODE_STRING InstancePath
    );

typedef PEPCALLBACKPREPAREDEVICE *PPEPCALLBACKPREPAREDEVICE;

typedef
BOOLEAN
PEPCALLBACKABANDONDEVICE (
    _In_ PCUNICODE_STRING InstancePath
    );

typedef PEPCALLBACKABANDONDEVICE *PPEPCALLBACKABANDONDEVICE;

typedef POWER_RUNTIME_COMPONENT_IDLE_STATE PEP_IDLE_STATE, *PPEP_IDLE_STATE;
typedef POWER_RUNTIME_COMPONENT_PERF_STATE PEP_PERF_STATE, *PPEP_PERF_STATE;
typedef POWER_RUNTIME_COMPONENT PEP_COMPONENT, *PPEP_COMPONENT;

struct _PEP_DEVICE_REGISTER {
    ULONG ComponentCount;
    _Field_size_full_(ComponentCount) PPEP_COMPONENT Components[ANYSIZE_ARRAY];
};

typedef struct _PEP_DEVICE_REGISTER PEP_DEVICE_REGISTER, *PPEP_DEVICE_REGISTER;

typedef
BOOLEAN
PEPCALLBACKREGISTERDEVICE (
    _In_ PCUNICODE_STRING InstancePath,
    _In_ POHANDLE KernelHandle,
    _In_ PPEP_DEVICE_REGISTER Register,
    _Out_ PEPHANDLE *DeviceHandle
    );

typedef PEPCALLBACKREGISTERDEVICE *PPEPCALLBACKREGISTERDEVICE;

typedef
VOID
PEPCALLBACKUNREGISTERDEVICE (
    _In_ PEPHANDLE DeviceHandle
    );

typedef PEPCALLBACKUNREGISTERDEVICE *PPEPCALLBACKUNREGISTERDEVICE;

enum _PEP_WORK_TYPE {
    PepWorkActiveComplete,
    PepWorkRequestFState,
    PepWorkPauseComponent,
    PepWorkSuspendComplete,
    PepWorkDevicePower,
    PepWorkMax
};

typedef enum _PEP_WORK_TYPE PEP_WORK_TYPE, *PPEP_WORK_TYPE;

struct _PEP_WORK_ACTIVECOMPLETE {
    POHANDLE DeviceHandle;
    ULONG Component;
};

typedef struct _PEP_WORK_ACTIVECOMPLETE PEP_WORK_ACTIVECOMPLETE, *PPEP_WORK_ACTIVECOMPLETE;

struct _PEP_WORK_FSTATE {
    POHANDLE DeviceHandle;
    ULONG Component;
    ULONG State;
};

typedef struct _PEP_WORK_FSTATE PEP_WORK_FSTATE, *PPEP_WORK_FSTATE;

struct _PEP_WORK_PAUSE_COMPONENT {
    POHANDLE DeviceHandle;
    ULONG Component;
    ULONG Performance;
    BOOLEAN Pause;
...
typedef struct _PEP_WORK_PAUSE_COMPONENT PEP_WORK_PAUSE_COMPONENT, *PPEP_WORK_PAUSE_COMPONENT;

struct _PEP_WORK_SUSPENDCOMPLETE {
    POHANDLE DeviceHandle;
};

typedef struct _PEP_WORK_SUSPENDCOMPLETE PEP_WORK_SUSPENDCOMPLETE, *PPEP_WORK_SUSPENDCOMPLETE;

struct _PEP_WORK_DEVICEPOWER {
    POHANDLE DeviceHandle;
    BOOLEAN PowerRequired;
};

typedef struct _PEP_WORK_DEVICEPOWER PEP_WORK_DEVICEPOWER, *PPEP_WORK_DEVICEPOWER;

struct _PEP_WORK_INFORMATION {
    PEP_WORK_TYPE WorkType;
    union {
        PEP_WORK_ACTIVECOMPLETE ActiveComplete;
        PEP_WORK_FSTATE FState;
        PEP_WORK_PAUSE_COMPONENT PauseComponent;
        PEP_WORK_SUSPENDCOMPLETE SuspendComplete;
        PEP_WORK_DEVICEPOWER DevicePower;
    };
};

typedef struct _PEP_WORK_INFORMATION PEP_WORK_INFORMATION, *PPEP_WORK_INFORMATION;

typedef
BOOLEAN
PEPCALLBACKCOMPONENTACTIVE (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ BOOLEAN Active,
    _Out_opt_ PPEP_WORK_INFORMATION WorkInformation
    );

typedef PEPCALLBACKCOMPONENTACTIVE *PPEPCALLBACKCOMPONENTACTIVE;
...
typedef PEPCALLBACKWORK *PPEPCALLBACKWORK;

typedef
BOOLEAN
PEPCALLBACKNOTIFYFSTATE (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ ULONG FState,
    _Out_opt_ PPEP_WORK_INFORMATION WorkInformation
...
typedef PEPCALLBACKNOTIFYFSTATE *PPEPCALLBACKNOTIFYFSTATE;

typedef
BOOLEAN
PEPCALLBACKCOMPONENTPAUSE (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ BOOLEAN Paused,
    _Out_opt_ PPEP_WORK_INFORMATION WorkInformation
...
typedef PEPCALLBACKCOMPONENTPAUSE *PPEPCALLBACKCOMPONENTPAUSE;

typedef
VOID
PEPCALLBACKCOMPONENTSETLATENCY (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ ULONG Latency
    );

typedef PEPCALLBACKCOMPONENTSETLATENCY *PPEPCALLBACKCOMPONENTSETLATENCY;

typedef
VOID
PEPCALLBACKCOMPONENTSETRESIDENCY (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ ULONG Residency
    );

typedef PEPCALLBACKCOMPONENTSETRESIDENCY *PPEPCALLBACKCOMPONENTSETRESIDENCY;

typedef 
VOID
PEPCALLBACKCOMPONENTSETWAKEHINT (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ BOOLEAN WakeHint
    );

typedef PEPCALLBACKCOMPONENTSETWAKEHINT *PPEPCALLBACKCOMPONENTSETWAKEHINT;

typedef
VOID
PEPCALLBACKDEVICESUSPEND (
    _In_ PEPHANDLE DeviceHandle,
    _In_ BOOLEAN Suspended
    );

typedef PEPCALLBACKDEVICESUSPEND *PPEPCALLBACKDEVICESUSPEND;

typedef
VOID
PEPCALLBACKDEVICEPOWER (
    _In_ PEPHANDLE PepHandle,
    _In_ BOOLEAN PowerRequired
    );

typedef PEPCALLBACKDEVICEPOWER *PPEPCALLBACKDEVICEPOWER;

typedef
BOOLEAN
PEPCALLBACKREQUESTPERFORMANCE (
    _In_ PEPHANDLE DeviceHandle,
    _In_ ULONG Component,
    _In_ ULONG Performance,
    _Out_opt_ PPEP_WORK_INFORMATION WorkInformation
...
typedef PEPCALLBACKREQUESTPERFORMANCE *PPEPCALLBACKREQUESTPERFORMANCE;

struct _PEP_CALLBACKS {
    USHORT Version;
    USHORT Size;
    PPEPCALLBACKPREPAREDEVICE PrepareDevice;
    PPEPCALLBACKABANDONDEVICE AbandonDevice;
    PPEPCALLBACKREGISTERDEVICE RegisterDevice;
    PPEPCALLBACKUNREGISTERDEVICE UnregisterDevice;
    PPEPCALLBACKCOMPONENTACTIVE ComponentActive;
    PPEPCALLBACKCOMPONENTPAUSE ComponentPause;
    PPEPCALLBACKCOMPONENTSETLATENCY ComponentSetLatency;
    PPEPCALLBACKCOMPONENTSETRESIDENCY ComponentSetResidency;
    PPEPCALLBACKCOMPONENTSETWAKEHINT ComponentSetWakeHint;
    PPEPCALLBACKWORK Work;
    PPEPCALLBACKNOTIFYFSTATE NotifyFState;
    PPEPCALLBACKDEVICESUSPEND DeviceSuspend;
    PPEPCALLBACKDEVICEPOWER DevicePower;
    PPEPCALLBACKREQUESTPERFORMANCE RequestPerformance;
};

typedef struct _PEP_CALLBACKS PEP_CALLBACKS, *PPEP_CALLBACKS;

typedef
NTSTATUS
PEPENTRY (
    _In_ PPEP_KERNEL_INFORMATION KernelInformation,
    _Out_ PPEP_CALLBACKS Callbacks
    );

typedef PEPENTRY *PPEPENTRY;
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
__drv_maxIRQL(PASSIVE_LEVEL)
_When_(Length==0, __drv_valueIs(<0))
_When_(Length>0, __drv_valueIs(<0;==0))
NTSYSAPI
NTSTATUS
NTAPI
ZwEnumerateKey(
...
typedef ULONG HYPERVISOR_PROCESSOR_INDEX, *PHYPERVISOR_PROCESSOR_INDEX;

#define HVL_INVALID_LP_INDEX    ((ULONG) -1)
...
HvlQueryActiveProcessors (
    ...
    _Out_writes_to_opt_(*Count, *Count) PHYPERVISOR_PROCESSOR_INDEX Processors    
    );

NTSTATUS
NTKERNELAPI
HvlQueryProcessorTopologyHighestId (
    _Out_opt_ PULONG HighestPackageId,
    _Out_opt_ PULONG HighestCoreId
    );

NTSTATUS
NTKERNELAPI
HvlQueryProcessorTopology (
    _In_ HYPERVISOR_PROCESSOR_INDEX LpIndex,
